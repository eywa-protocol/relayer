...
func (m *FBFTMessage) String() string {
	...
	for _, key := range m.SenderPubkeys {
		if sender == "" {
			sender = common.Bytes2Hex(key.Marshal())
		} else {
			sender = sender + ";" + common.Bytes2Hex(key.Marshal())
		}
	}
	leader := ""
	if m.LeaderPubkey != nil {
		leader = common.Bytes2Hex(m.LeaderPubkey.Marshal())
	}
	return fmt.Sprintf(
		"[ViewID:%d Num:%d BlockHash:%s Sender:%s Leader:%s]",
		m.ViewID,
		m.BlockNum,
		m.BlockHash.Hex(),
		sender,
...
const (
	...
	idSenderBytes = PublicKeySizeInBytes
...
func (m *FBFTMessage) id() fbftMsgID {
	...
	if m.HasSingleSender() {
		copy(id[idTypeBytes+idViewIDBytes+idHashBytes:], m.SenderPubkeys[0].Marshal()[:])
	} else {
...
func (log *FBFTLog) AddBlock(block *types.Block) {
	...
	log.blocks[(common.Hash)(block.Hash())] = block
}
...
func (log *FBFTLog) MarkBlockVerified(block *types.Block) {
	...
	log.verifiedBlocks[(common.Hash)(block.Hash())] = struct{}{}
}
...
func (log *FBFTLog) GetNotVerifiedCommittedMessages(blockNum uint64, viewID uint64, blockHash common.Hash) []*FBFTMessage {
	...
	for _, msg := range log.messages {
		if msg.MessageType == message.MsgType_Commit && msg.BlockNum == blockNum && msg.ViewID == viewID && msg.BlockHash == blockHash && !msg.Verified {
...
func (log *FBFTLog) GetMessagesByTypeSeqViewHash(typ message.MsgType, blockNum uint64, viewID uint64, blockHash common.Hash) []*FBFTMessage {
	log.msgLock.RLock()
	defer log.msgLock.RUnlock()
...
func (log *FBFTLog) GetMessagesByTypeSeq(typ message.MsgType, blockNum uint64) []*FBFTMessage {
	log.msgLock.RLock()
	defer log.msgLock.RUnlock()
...
func (log *FBFTLog) GetMessagesByTypeSeqHash(typ message.MsgType, blockNum uint64, blockHash common.Hash) []*FBFTMessage {
	log.msgLock.RLock()
	defer log.msgLock.RUnlock()
...
func (log *FBFTLog) HasMatchingAnnounce(blockNum uint64, blockHash common.Hash) bool {
	found := log.GetMessagesByTypeSeqHash(message.MsgType_Announce, blockNum, blockHash)
	return len(found) >= 1
}
...
func (log *FBFTLog) HasMatchingViewAnnounce(blockNum uint64, viewID uint64, blockHash common.Hash) bool {
	found := log.GetMessagesByTypeSeqViewHash(message.MsgType_Announce, blockNum, viewID, blockHash)
	return len(found) >= 1
}
...
func (log *FBFTLog) HasMatchingViewPrepared(blockNum uint64, viewID uint64, blockHash common.Hash) bool {
	found := log.GetMessagesByTypeSeqViewHash(message.MsgType_Prepared, blockNum, viewID, blockHash)
	return len(found) >= 1
}

// GetMessagesByTypeSeqView returns pbft messages with matching type, blockNum and viewID
func (log *FBFTLog) GetMessagesByTypeSeqView(typ message.MsgType, blockNum uint64, viewID uint64) []*FBFTMessage {
	log.msgLock.RLock()
	defer log.msgLock.RUnlock()
...
func (consensus *Consensus) ParseFBFTMessage(msg *message.ConsensusRequest) (*FBFTMessage, error) {
	pbftMsg := FBFTMessage{}
	pbftMsg.MessageType = *msg.MsgType
...
func (log *FBFTLog) GetCommittedBlockAndMsgsFromNumber(bn uint64) (*types.Block, *FBFTMessage, error) {
	msgs := log.GetMessagesByTypeSeq(
		message.MsgType_Commited, bn,
	)
	if len(msgs) == 0 {
		return nil, nil, errFBFTLogNotFound
	}
	if len(msgs) > 1 {
		logrus.Errorf("DANGER!!! multiple COMMITTED message in PBFT log observed numMsgs: %d", len(msgs))
	}
	for i := range msgs {
		block := log.GetBlockByHash(msgs[i].BlockHash)
		if block == nil {
			logrus.Errorf("failed finding a matching block for committed message. blockNum %v viewID %v blockHash %v", msgs[i].BlockNum, msgs[i].ViewID, msgs[i].BlockHash.Hex())
			continue
		}
