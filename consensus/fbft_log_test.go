...
func TestFBFTLog_id(t *testing.T) {
	...
	tests := []FBFTMessage{
		{
			MessageType:        message.MsgType_Announce,
			ViewID:             4,
			BlockHash:          [32]byte{01, 02},
			SenderPubkeys:      []*bls.PublicKey{&pub},
			SenderPubkeyBitmap: []byte{05, 07},
		},
		{
			MessageType:        message.MsgType_Commit,
			ViewID:             4,
	...
	for _, msg := range tests {
		...
		if !msg.HasSingleSender() {
			...
			if !bytes.Equal(id[idTypeBytes+idViewIDBytes+idHashBytes:], msg.SenderPubkeys[0].Marshal()) {
...
func testGetMessagesByTypeSeqViewHash(t *testing.T) {
	...
	found := log.GetMessagesByTypeSeqViewHash(
		message.MsgType_Commit, 2, 3, [32]byte{01, 02},
	)
	if len(found) != 1 {
	...
	notFound := log.GetMessagesByTypeSeqViewHash(
		message.MsgType_Announce, 2, 3, [32]byte{01, 03},
	)
	if len(notFound) > 0 {
